...
describe('crypto utilities', () => {
  ...
  it('should throw an error for an incorrect PIN', async () => {
    ...
    const pin = '1234';
    const encrypted = await encryptToken(token, pin);
    await expect(decryptToken(encrypted, '0000')).rejects.toThrow('Invalid PIN or corrupted data');
  });
  ...
  it('should throw an error for corrupted data', async () => {
    ...
    await expect(decryptToken(corrupted, pin)).rejects.toThrow('Invalid PIN or corrupted data');
  });
});
