...
import {
  Search, TrendingUp, DollarSign, Percent, Shield, Clock, CheckCircle2, ChevronRight
} from 'lucide-react';
...
export function DynamicRateEngine() {
  ...
  const calculatedRates = useMemo(() => {
    return mockRates.map(rate => {
      const buyRateTotal = rate.baseOceanFreight + rate.baf + rate.pss + rate.thc;
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 bg-[#16161A] border border-gray-800/80 rounded-xl p-5 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-[11px] text-gray-500 mb-1 font-medium">Origen (POL)</label>
                <input 
                  type="text" 
                  value={origin} 
              ...
              <div>
                <label className="block text-[11px] text-gray-500 mb-1 font-medium">Destino (POD)</label>
                <input 
                  type="text" 
                  value={destination} 
              ...
              <div>
                <label className="block text-[11px] text-gray-500 mb-1 font-medium">Equipo</label>
                <select 
                  value={containerSize} 
          ...
          <div className="w-full lg:w-96 bg-gradient-to-br from-indigo-950/40 to-[#16161A] border border-indigo-500/20 rounded-xl p-5 shadow-lg relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              <div className="flex p-1 bg-[#0A0A0B] rounded-lg border border-gray-800/80 w-fit">
                <button 
                  onClick={() => { setMarginType('flat'); setMarginValue(300); }}
                ...
                <button 
                  onClick={() => { setMarginType('percentage'); setMarginValue(15); }}
              ...
              <div>
                <label className="block text-[11px] text-indigo-200/60 mb-1 font-medium">Margen a aplicar</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    {marginType === 'flat' ? <DollarSign className="w-4 h-4 text-indigo-400" /> : <Percent className="w-4 h-4 text-indigo-400" />}
                  </div>
                  <input 
                    type="number" 
                    value={marginValue} 
        ...
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="flex justify-between items-end mb-4">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Ruta: <span className="text-gray-200 font-medium">{origin} <ChevronRight className="inline w-3 h-3" /> {destination}</span></p>
            </div>
          </div>
              ...
              <thead>
                <tr className="bg-[#16161A] border-b border-gray-800 text-[10px] uppercase tracking-widest text-gray-500 font-bold">
                  ...
                  <th className="py-4 px-5 text-right group relative cursor-help">
                    <span className="border-b border-dashed border-gray-600">Recargos</span>
                    {/* Tooltip */}
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 bg-gray-800 text-gray-200 text-xs p-3 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                      Incluye BAF (Bunker), PSS (Peak Season) y THC Origen/Destino.
                    </div>
                  </th>
                  <th className="py-4 px-5 text-right text-indigo-400 bg-indigo-950/20">Margen Profit</th>
                  <th className="py-4 px-5 text-right font-bold text-white bg-blue-950/20">Tarifa Venta (Total)</th>
                  <th className="py-4 px-5 text-center">Acción</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800/60">
                {calculatedRates.map((rate, idx) => (
                  <tr key={rate.id} className={`hover:bg-[#16161A] transition-colors ${idx === 0 ? 'bg-emerald-950/10' : ''}`}>
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-white shadow-inner ${
                          rate.carrier === 'Maersk' ? 'bg-blue-600' :
                          rate.carrier === 'MSC' ? 'bg-yellow-600' :
                          rate.carrier === 'Hapag-Lloyd' ? 'bg-orange-600' : 'bg-red-600'
                        }`}>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-2">
                            ...
                            <span>•</span>
                            <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {rate.transitTime} días</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-5 text-right font-mono text-sm text-gray-300">
                      ${rate.baseOceanFreight.toLocaleString()}
                    </td>
                    ...
                    <td className="py-4 px-5 text-right bg-indigo-950/10">
                      <div className="font-mono text-sm font-bold text-indigo-400">
                        +${rate.sellMargin.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                      <div className="text-[10px] text-indigo-500/60 mt-0.5">
                        {marginType === 'percentage' ? `${marginValue}% markup` : 'Flat margin'}
                      </div>
                    </td>
                    
                    <td className="py-4 px-5 text-right bg-blue-950/10">
                      <div className="font-mono text-lg font-bold text-white">
                        ${rate.sellRateTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                    ...
                    <td className="py-4 px-5 text-center">
                      <button className="p-2 bg-gray-800 hover:bg-blue-600 hover:text-white text-gray-400 rounded-lg transition-colors" title="Generar Cotización PDF">
