...
export function RateTable({ rates, onSelectRate }: RateTableProps) {
  ...
  const formatCurrency = (amount: number) => {
    const converted = selectedCurrency === 'EUR' ? amount * exchangeRate : amount;
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-xl overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-slate-800 bg-slate-800/30 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-400">Compare carrier rates, surcharges, and apply margins.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900/50 text-slate-400 text-xs uppercase tracking-wider border-b border-slate-800">
              ...
              <th className="p-4 font-medium text-right text-blue-400 bg-blue-900/10">Base Rate</th>
              <th className="p-4 font-medium text-right text-amber-400 bg-amber-900/10">Surcharges (BAF+PSS)</th>
              <th className="p-4 font-medium text-right text-purple-400 bg-purple-900/10">Total Buy</th>
              <th className="p-4 font-medium text-right text-emerald-400 bg-emerald-900/10">Sell Margin</th>
              <th className="p-4 font-medium text-right text-emerald-400 bg-emerald-900/20 font-bold">Client Quote</th>
            ...
            {rates.map(rate => {
              ...
              return (
                <tr key={rate.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="p-4">
                    <span className="font-bold text-slate-200">{rate.carrier}</span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-sm text-slate-400">{rate.transitTime}</td>
                  ...
                  <td className="p-4 text-sm text-right text-amber-300 bg-amber-900/5">
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-[10px] text-slate-500">BAF: {rate.baf} | PSS: {rate.pss}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-sm text-right bg-emerald-900/5">
                    ...
                    <div className="text-[10px] text-slate-500">+{formatCurrency(marginAmount)} Profit</div>
                  </td>
                  <td className="p-4 text-right bg-emerald-900/10">
                    <span className="text-lg font-bold text-emerald-400">{formatCurrency(totalSell)}</span>
                  </td>
