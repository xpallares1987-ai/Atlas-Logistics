...
export interface ESGCarbonTrackerProps {
  data?: ShipmentCarbon[];
  isLoading?: boolean;
}
...
export function ESGCarbonTracker({
  ...
  const metrics = useMemo(() => {
    ...
    const pieData = Object.entries(modeBreakdown)
      .map(([name, value]) => ({
        name,
        value,
      }))
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-2 max-w-2xl">
            Live monitoring of Greenhouse Gas (GHG) Protocol Scope 3 emissions
            across your active supply chain network.
          </p>
        </div>
        <motion.button
        ...
        >
          ...
          <div className="flex justify-center gap-8 mt-4">
            {Object.keys(COLORS).map((mode) => {
              const Icon =
                mode === "Ocean" ? Anchor : mode === "Air" ? Wind : Truck;
              return (
                <div key={mode} className="flex items-center gap-2">
                  <div
                    className="p-1.5 rounded-md"
                    style={{
                      backgroundColor: `${COLORS[mode as keyof typeof COLORS]}30`,
                    }}
                  >
                    <Icon
                      size={16}
                      color={COLORS[mode as keyof typeof COLORS]}
                    />
                  </div>
              ...
              <AnimatePresence>
                {shipments.map((s) => (
                  ...
                  >
                    ...
                    <TableCell className="font-black text-emerald-400 text-right group-hover:text-emerald-300 font-mono text-base">
                      {s.co2eTonnes.toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
