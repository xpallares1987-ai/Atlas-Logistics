...
export function ContainerScene({ container, packedItems, viewMode }: ContainerSceneProps) {
  // Determine camera position based on view mode
  const cameraConfig = useMemo(() => {
    if (viewMode === 'top') {
      return { position: [0, Math.max(container.length, container.width) * 1.5, 0], fov: 45 };
    }
    if (viewMode === 'side') {
      return { position: [0, container.height / 2, Math.max(container.length, container.width) * 1.5], fov: 45 };
    }
    // Default 3D Isometric
    return { position: [container.length * 0.8, container.height * 1.5, container.width * 1.2], fov: 45 };
  }, [viewMode, container]);

  return (
    <Canvas camera={{ position: cameraConfig.position as [number, number, number], fov: cameraConfig.fov }}>
      ...
      <group position={[-container.length / 2, 0, -container.width / 2]}>
        ...
        <mesh position={[container.length / 2, container.height / 2, container.width / 2]}>
          ...
          <meshBasicMaterial color="#a0aec0" transparent opacity={0.05} depthWrite={false} side={THREE.DoubleSide} />
          <Edges scale={1} threshold={15} color="#cbd5e1" />
        </mesh>

        {/* Floor Grid representing Container floor */}
        <gridHelper 
          args={[Math.max(container.length, container.width) * 2, Math.max(container.length, container.width) * 2, '#475569', '#334155']} 
        ...
        {packedItems.map((item) => {
          ...
          return (
            <mesh key={item.id} position={[px, py, pz]}>
              ...
              <meshStandardMaterial color={item.color || '#3b82f6'} roughness={0.4} metalness={0.1} />
