...
export function DemurrageAlerts() {
  ...
  // Fetch from API
  const { data: containers = [], isLoading } = useQuery<ContainerAlert[]>({
    queryKey: ["demurrage"],
    queryFn: async () => {
      // Hardcode relative path since ui package isn't built with Vite envs
  ...
  const mitigateMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const res = await fetch(`/api/operations/demurrage/mitigate`, {
        method: "POST",
  ...
  const openEmailDraft = (c: ContainerAlert) => {
    ...
    setEmailTo(`import.ops@${c.carrier.toLowerCase().replace(/\s+/g, "")}.com`);
    setEmailSubject(
              ...
              <div className="mt-auto bg-gradient-to-br from-amber-500/10 to-red-500/10 border border-amber-500/20 p-4 rounded-2xl flex gap-3 items-start backdrop-blur-md">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs leading-relaxed font-medium">
                    Las navieras aplican recargos exponenciales tras agotar el
                    plazo libre. Priorice la retirada del contenedor de{" "}
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="p-6 border-t border-white/5 bg-slate-800/20 flex justify-end gap-3">
              ...
              <button
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm bg-red-600 hover:bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.3)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={handleSendEmail}
                disabled={emailSending}
              >
                <Send size={16} />{" "}
                {emailSending ? "Enviando..." : "Enviar Solicitud"}
              </button>
            </div>
          </div>
        </div>
