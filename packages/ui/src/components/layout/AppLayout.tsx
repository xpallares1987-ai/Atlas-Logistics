...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-300 font-sans overflow-hidden selection:bg-indigo-500/30">
      {/* LEFT SIDEBAR */}
      <aside
        className={`bg-slate-900 border-r border-slate-800 flex flex-col transition-all duration-300 ${sidebarCollapsed ? "w-20" : "w-64"} shrink-0 z-20`}
      >
        {/* Brand Header */}
        <div className="h-14 flex items-center justify-between px-4 border-b border-slate-800 bg-slate-950/50">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!sidebarCollapsed && (
              <span className="font-black text-white tracking-wide truncate">
                ATLAS SCM
              </span>
            )}
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon
                  className={`w-5 h-5 shrink-0 transition-transform ${isActive ? "scale-110" : "group-hover:scale-110"}`}
                />
                {!sidebarCollapsed && (
          ...
          <div className="flex-1 flex items-end gap-1 overflow-x-auto hide-scrollbar h-full pt-2">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div className="absolute top-0 left-0 w-full h-0.5 bg-indigo-500 rounded-t-xl" />
                  )}
        ...
        <aside className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col shrink-0 z-30 shadow-2xl absolute right-0 top-0 h-full transform transition-transform duration-300">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="text-center text-slate-500 text-sm mt-10">
              ...
              <p>
                Select a shipment or quote to see context, activity logs, and
                chat.
              </p>
            </div>
          </div>
