...
import { 
  AlertTriangle, CheckCircle, ShieldAlert, Mail,
  Search, X, Send
  } from 'lucide-react';
...
export function DemurrageAlerts() {
  ...
  const [selectedContainer, setSelectedContainer] = useState<ContainerAlert | null>(null);
  ...
  const openEmailDraft = (c: ContainerAlert) => {
    ...
    setEmailSubject(`URGENT: Demurrage Free-Time Extension Request - ${c.container} / ${c.reference}`);
  ...
  return (
    <div className="demurrage-dashboard">
      ...
      <div className="report-kpi-grid" style={{ marginBottom: '2rem' }}>
        ...
        <div className="kpi-panel-card" style={{ borderLeft: '4px solid #ef4444' }}>
        ...
        <div className="kpi-panel-card" style={{ borderLeft: '4px solid #f59e0b' }}>
        ...
        <div className="kpi-panel-card" style={{ borderLeft: '4px solid #3b82f6' }}>
        ...
        <div className="kpi-panel-card" style={{ borderLeft: '4px solid #10b981' }}>
          <span className="kpi-label">Dwell Time Promedio</span>
          <h2 className="kpi-value" style={{ color: '#10b981' }}>
            {metrics.avgDwell} días
          </h2>
          <span className="kpi-detail">Estancia media en terminal portuaria</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '2rem' }}>
        ...
        <div className="card table-card" style={{ marginBottom: 0 }}>
          <div className="card-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <ShieldAlert className="text-accent" style={{ color: '#ef4444' }} />
            ...
            <table className="custom-report-table">
              ...
              <tbody>
                {filteredContainers.map(c => {
                  ...
                  return (
                    <tr key={c.id}>
                      <td className="bold" style={{ fontFamily: 'monospace' }}>{c.container}</td>
                      <td>{c.carrier}</td>
                      <td>{c.pod}</td>
                      <td>{c.portArrivalDate}</td>
                      <td><strong>{c.dwellDays} días</strong></td>
                      ...
                      <td>
                        {exposure > 0 ? (
                          <strong style={{ color: '#ef4444' }}>USD {exposure.toLocaleString('es-ES')}</strong>
                        ) : (
                          <span className="text-success" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.85rem' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: 0 }}>
          ...
          <div className="carrier-bar-chart-list" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {carrierRiskData.map((carrier, idx) => {
              // Calculate percent width of progress bar based on highest exposure
              const maxExposure = Math.max(...carrierRiskData.map(d => d.exposure), 1);
              const percent = (carrier.exposure / maxExposure) * 100;
              
              return (
                <div key={idx} className="carrier-chart-row" style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', fontWeight: 600 }}>
                    <span>{carrier.name} ({carrier.count} cont.)</span>
                    <strong style={{ color: carrier.exposure > 0 ? '#ef4444' : 'var(--text-muted)' }}>
                      USD {carrier.exposure.toLocaleString('es-ES')}
                    </strong>
                  </div>
                  <div className="custom-progress-bar-bg" style={{ width: '100%', height: '10px', background: 'var(--bg-tertiary)', borderRadius: '999px', overflow: 'hidden' }}>
                    <div 
                      className="custom-progress-bar-fill" 
                      style={{ 
                        width: `${percent}%`, 
                        height: '100%', 
                        background: carrier.exposure > 1000 ? 'linear-gradient(90deg, #f59e0b 0%, #ef4444 100%)' : carrier.exposure > 0 ? '#f59e0b' : '#10b981', 
                        borderRadius: '999px',
          ...
          <div className="card-notes-panel" style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.05) 0%, rgba(239, 68, 68, 0.05) 100%)', border: '1px solid rgba(245, 158, 11, 0.2)', padding: '1rem', borderRadius: '12px', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
            <AlertTriangle className="text-warning" size={24} style={{ flexShrink: 0, marginTop: '0.1rem', color: '#f59e0b' }} />
            <div style={{ fontSize: '0.85rem', lineHeight: '1.4' }}>
              <h4 style={{ fontWeight: 'bold', marginBottom: '0.25rem', color: '#f59e0b' }}>Protocolo de Retención D&D</h4>
              <p className="text-muted">Las navieras aplican recargos exponenciales tras agotar el plazo libre. Priorice la retirada del contenedor de <strong>{carrierRiskData[0]?.name || 'la naviera principal'}</strong> para frenar pérdidas diarias.</p>
            </div>
          </div>
        </div>
      ...
      {emailModalOpen && selectedContainer && (
        <div className="sheet-picker-modal-backdrop">
          <div className="sheet-picker-modal" style={{ maxWidth: '650px', width: '95%' }}>
            
            <div className="modal-header" style={{ justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <Mail size={24} className="text-accent" style={{ color: '#ef4444' }} />
            ...
            <div className="email-form-fields" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-muted)' }}>Destinatario:</label>
                <input 
                  type="text" 
                  className="search-input" 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-muted)' }}>Asunto del Email:</label>
                <input 
                  type="text" 
                  className="search-input" 
                  style={{ padding: '0.6rem 1rem' }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-muted)' }}>Cuerpo de la Solicitud:</label>
                <textarea 
                  className="search-input"
                  style={{ 
                    padding: '1rem', 
            ...
            <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
              <button className="btn-secondary" onClick={() => setEmailModalOpen(false)}>Cancelar</button>
              <button className="btn-primary" style={{ background: '#ef4444', display: 'flex', alignItems: 'center', gap: '0.5rem' }} onClick={handleSendEmail}>
