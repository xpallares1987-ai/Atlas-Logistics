...
const XMLSerializerToUse =
  typeof self.XMLSerializer !== "undefined"
    ? self.XMLSerializer
    : PolyfilledXMLSerializer;
...
function extractStats(doc: Document) {
  ...
  const isCamunda8 =
    doc.documentElement.getAttribute("xmlns:zeebe") !== null ||
...
function generateSopFromDoc(doc: Document): string {
  ...
  tasks.forEach((task, idx) => {
    ...
    if (extElements.length > 0) {
      const taskDefs = extElements[0].getElementsByTagNameNS(
        "*",
        "taskDefinition",
      );
  ...
  try {
    ...
    if (action === "execute-process") {
      const tasks = Array.from(
        doc.getElementsByTagNameNS("*", "serviceTask"),
      ).map((task) => ({
    ...
    const message =
      error instanceof Error
        ? error.message
        : "An unknown error occurred during BPMN processing.";
