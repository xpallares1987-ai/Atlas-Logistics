...
export function safeGetMetadata(bo: any): SysMetadata {
  ...
  if (!result.success) {
    console.warn(
      "Metadata validation failed, using defaults:",
      result.error.format(),
    );
...
export function validateProperty(key: string, value: any): any {
  ...
  const partialSchema = SysMetadataSchema.pick({
    [schemaKey as any]: true,
  } as any);
  ...
  if (!result.success) {
    const firstError =
      result.error.issues[0]?.message || `Valor inválido para ${key}`;
