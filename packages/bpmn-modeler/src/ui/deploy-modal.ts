...
export function initDeployModal(statusbar: Statusbar, state: AppState) {
  ...
  btnDeploy.addEventListener("click", () => modal.showModal());
  ...
  deployForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const isSandbox = (
      document.getElementById("zeebeSandbox") as HTMLInputElement
    )?.checked;
    ...
    try {
      if (isSandbox) {
        const stages = [
          { text: "Estableciendo conexión con Atlas Engine...", delay: 1200 },
          { text: "Autenticando credenciales de usuario...", delay: 1400 },
          { text: "Validando sintaxis XML...", delay: 1000 },
          { text: "Enviando comando de despliegue...", delay: 1200 },
          { text: "¡Despliegue exitoso!", delay: 600 },
        ];
            ...
            const workerPromise = new Promise((resolve, reject) => {
              worker.onmessage = (event) => {
                if (
                  ...
                  event.data.action === "execute-process"
                ) {
        ...
        statusbar.setStatus(
          `Atlas Engine: Desplegado v${randomVersion}`,
          "success",
        );
      } else {
      ...
      Toast.show(
        error.message || "Fallo crítico al desplegar en Atlas Engine",
        "error",
      );
      resetForm();
    }
