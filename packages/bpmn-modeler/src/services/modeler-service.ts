...
import { publishEvent } from '@atlas/ui';
...
export async function createModeler({
  ...
  try {
    ...
    if (camunda8 && zeebeSupport) {
      const { default: ZeebeBehaviorsModule } =
        await import('camunda-bpmn-js-behaviors/lib/camunda-cloud');
    ...
    // Initialize custom keyboard shortcuts
    initCustomKeyboard(modeler);

    // Hook to intercept changes
    const eventBus = modeler.get('eventBus') as EventBus;
    let autoSaveTimeout: ReturnType<typeof setTimeout> | null = null;

    eventBus.on('commandStack.changed', async () => {
      console.log('Command stack changed, broadcasting remote change');
      const xml = await getDiagramXml(modeler);
      publishEvent({ type: 'DIAGRAM_CHANGED', payload: xml });

      // Advanced Undo/Redo: Debounced Autosave to HistoryService
      ...
      autoSaveTimeout = setTimeout(async () => {
        ...
        if (activeTab) {
          ...
          console.log(`[History] Version checkpoint saved for tab: ${activeTab.id}`);
        }
      }, 2000);
    });
...
// --- Modified importDiagram function ---
export async function importDiagram(modeler: Modeler, xml: string) {
  ...
  try {
    ...
    console.error('Error importing diagram:', importError);
    throw new Error(
      `Failed to import diagram: ${importError instanceof Error ? importError.message : ''}`
    );
  }
}
...
export function toggleMinimap(modeler: Modeler) {
  ...
  try {
    ...
    console.warn('Minimap module not found or failed to toggle');
  }
}
...
export function searchElements(modeler: Modeler, term: string): BpmnElement[] {
  ...
  return elementRegistry.filter((element) => {
    ...
    const nameMatch =
      businessObject &&
      businessObject.name &&
      businessObject.name.toLowerCase().includes(termLower);
  ...
  try {
    ...
    for (const element of allElements) {
      ...
      if (bo && (bo.$instanceOf('bpmn:Task') || (bo.$type && bo.$type.includes('Task')))) {
        ...
        if (metadata.status === 'delayed' || metadata.status === 'Retrasado') {
          result.delayedTasksCount++;
          result.bottlenecks.push({
            id: element.id,
            name,
            cost: metadata.costHR,
            status: 'Retrasado',
          });
        } else if (metadata.status === 'blocked' || metadata.status === 'Bloqueado') {
    ...
    console.warn('Error computing process analytics:', err);
  }
