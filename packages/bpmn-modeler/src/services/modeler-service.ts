...
export async function createModeler({
  ...
  try {
    ...
    if (propertiesPanel) {
      const { loadPropertiesModules } =
        await import("./properties-panel-loader");
...
interface BpmnCanvas {
  zoom(
    level: string | number,
    center?: BpmnElement | { x: number; y: number } | "auto",
  ): void;
}
...
export function calculateProcessAnalytics(
  ...
  try {
    ...
    for (const element of allElements) {
      ...
      if (
        bo &&
        (bo.$instanceOf("bpmn:Task") || (bo.$type && bo.$type.includes("Task")))
      ) {
        result.totalTasks++;
