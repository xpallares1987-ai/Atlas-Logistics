...
const IsometricWarehouse = ({ traffic, docks = 8 }: { traffic: any[]; docks?: number }) => {
  return (
      ...
      <div className="relative w-full h-full [transform:rotateX(60deg)_rotateZ(-45deg)] transition-transform duration-1000 group-hover:[transform:rotateX(55deg)_rotateZ(-40deg)] flex flex-col justify-center items-center">
        ...
        <div className="relative w-[400px] h-[300px] bg-slate-800/80 border border-slate-600/50 rounded-xl shadow-[20px_20px_40px_rgba(0,0,0,0.5)] flex flex-col justify-between p-4">
          ...
          <div className="flex justify-between w-full h-2/3">
            {[1, 2, 3].map((rack) => (
              <div key={rack} className="w-16 h-full bg-slate-700/50 border border-slate-600/30 rounded-lg flex flex-col justify-around p-1">
                {[1, 2, 3, 4].map((shelf) => (
                  <div key={shelf} className="w-full h-3 bg-emerald-500/20 rounded shadow-[inset_0_0_10px_rgba(16,185,129,0.2)]" />
                ))}
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-between w-full h-1/4 pt-4 border-t border-slate-700/50 gap-2">
            {Array.from({ length: docks }).map((_, i) => {
              ...
              const assignedTraffic = traffic?.find(t => t.assignedDock === dockNum && t.status !== "Completed");
              const isOccupied = !!assignedTraffic;

              return (
                <div key={i} className="relative w-full h-full flex items-end justify-center">
                  <div className={`w-8 h-8 border-2 rounded ${isOccupied ? 'border-amber-500 bg-amber-500/20' : 'border-emerald-500 bg-emerald-500/10'}`}>
                    {isOccupied && (
                       <motion.div 
                  ...
                  <span className="absolute -top-4 text-[8px] font-mono text-slate-400 [transform:rotateZ(45deg)]">{dockNum}</span>
                </div>
              );
            })}
          </div>
...
export default function WarehouseOpsModule() {
  const [activeTab, setActiveTab] = useState<"traffic" | "inventory" | "map">("traffic");
  ...
  const activeVehicles = traffic.filter((t: any) => t.status !== "Completed").length;
  const totalPallets = inventory.reduce((acc: number, curr: any) => acc + (curr.quantity || 0), 0);
        ...
        <div className="max-w-[1600px] mx-auto">
          <div className="mb-8">
            ...
            <p className="text-slate-400 font-medium max-w-2xl">
              Real-time Traffic Control, Cross-Docking Management, and Live Inventory Tracking.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col gap-2 relative overflow-hidden group">
              ...
              <span className="text-5xl font-black text-white z-10 tracking-tighter">{activeVehicles}</span>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col gap-2 relative overflow-hidden group">
              ...
              <span className="text-5xl font-black text-white z-10 tracking-tighter">{totalPallets}</span>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col gap-2 relative overflow-hidden group">
              ...
              <span className="text-5xl font-black text-white z-10 tracking-tighter">
                {8 - activeVehicles} <span className="text-2xl text-slate-500">/ 8</span>
              </span>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col gap-2 relative overflow-hidden group">
              ...
              <span className="text-5xl font-black text-white z-10 tracking-tighter">94%</span>
            ...
            <AnimatePresence mode="wait">
              {activeTab === "traffic" && (
                ...
                >
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-white">Inbound & Outbound Traffic</h2>
                  </div>
                  
                  {loadingTraffic ? (
                    <div className="flex justify-center py-20"><div className="animate-spin w-8 h-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div>
                  ) : (
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-white/10 text-slate-400 text-xs uppercase tracking-wider">
                            ...
                            <th className="p-4 font-semibold">ETA / Assigned Dock</th>
                            <th className="p-4 font-semibold">Cargo Expected</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                          {traffic.map((t: any) => (
                            <tr key={t.id} className="hover:bg-white/5 transition-colors group">
                              <td className="p-4">
                                <div className="font-bold text-white">{t.deviceNumber || 'Unknown'}</div>
                                <div className="text-sm text-slate-400">{t.driverName}</div>
                              </td>
                              <td className="p-4">
                                <span className={`px-2.5 py-1 text-xs font-bold rounded-lg border ${
                                  t.type === 'INBOUND' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' 
                              ...
                              <td className="p-4">
                                <div className="flex items-center gap-2">
                                  <div className={`w-2 h-2 rounded-full ${
                                    t.status === 'Arriving' ? 'bg-amber-400' :
                                    t.status === 'At Dock' ? 'bg-emerald-400' :
                                    'bg-slate-400'
                                  }`} />
                                  <span className="text-sm text-slate-300 font-medium">{t.status}</span>
                                </div>
                              </td>
                              <td className="p-4">
                                <div className="font-mono text-sm text-amber-400 font-bold">{t.assignedDock || 'PENDING'}</div>
                                {t.eta && <div className="text-xs text-slate-500">{new Date(t.eta).toLocaleTimeString()}</div>}
                              </td>
                              <td className="p-4">
                                <div className="text-sm text-white font-medium">{t.expectedQuantity} Units</div>
                                <div className="text-xs text-slate-400 truncate max-w-[200px]">{t.cargoDescription}</div>
                              </td>
                            </tr>
                          ))}
                          {traffic.length === 0 && (
                            <tr><td colSpan={5} className="p-8 text-center text-slate-500">No active traffic recorded.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
              ...
              {activeTab === "inventory" && (
                ...
                >
                  ...
                  {loadingInventory ? (
                    <div className="flex justify-center py-20"><div className="animate-spin w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full" /></div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                      {inventory.map((inv: any) => (
                        <div key={inv.id} className="bg-slate-900/80 border border-white/10 rounded-2xl p-5 hover:border-emerald-500/50 transition-colors group">
                          ...
                          <h3 className="font-bold text-white text-base mb-1 truncate" title={inv.sku}>
                            SKU: {inv.sku}
                          </h3>
                          <p className="text-sm text-slate-400 mb-4">{inv.customer || 'Unknown Customer'}</p>
                          <div className="pt-4 border-t border-white/5 flex justify-between items-center">
                            <div>
                              <div className="text-xs text-slate-500 mb-0.5">Quantity</div>
                              <div className="font-bold text-emerald-400">{inv.quantity} <span className="text-slate-400 font-normal text-xs">{inv.uom}</span></div>
                            </div>
                            {inv.weight && (
                              <div className="text-right">
                                <div className="text-xs text-slate-500 mb-0.5">Weight</div>
                                <div className="font-bold text-white">{inv.weight} kg</div>
                              </div>
                            )}
                          </div>
                        </div>
              ...
              {activeTab === "map" && (
                ...
                >
                  <div className="mb-4">
                    <h2 className="text-xl font-bold text-white">Terminal Isometric View</h2>
                    <p className="text-sm text-slate-400">Live visualization of dock utilization and warehouse zoning.</p>
                  </div>
