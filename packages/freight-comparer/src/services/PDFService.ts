...
export async function generateExecutiveReport(
  ...
  doc.text(
    `Origin: ${searchParams.origin || "All"} | Destination: ${searchParams.destination || "All"}`,
    14,
    33,
  );
  ...
  // Summary Metrics
  const avgRate =
    rates.reduce((acc, r) => acc + (r.total || 0), 0) / (rates.length || 1);
 ...
 */
export async function generateCustomerQuote(
  rate: FreightRate,
  ...
  doc.text(
    `Transit Time: ${rate.transitTime ? `${rate.transitTime} Days` : "Subject to confirmation"}`,
    14,
    77,
  );
  ...
  if (rate.baf && rate.baf > 0)
    tableData.push([
      "BAF (Bunker Adjustment)",
      formatCurrency(rate.baf, "USD"),
    ]);
  if (rate.thc && rate.thc > 0)
    tableData.push([
      "THC (Terminal Handling)",
      formatCurrency(rate.thc, "USD"),
    ]);
  if (rate.lss && rate.lss > 0)
    tableData.push(["LSS (Low Sulfur)", formatCurrency(rate.lss, "USD")]);
  if (rate.otrosRecargos && rate.otrosRecargos > 0)
    tableData.push([
      "Other Surcharges",
      formatCurrency(rate.otrosRecargos, "USD"),
    ]);
