...
export default function RateTable({ rates }: RateTableProps) {
  ...
  const bookedRates = useSelector(
    (state: RootState) => state.booking.bookedRates,
  );
  ...
  const handleBooking = async (rate: FreightRateMock) => {
    ...
    try {
      ...
      const eta = new Date(
        Date.now() + rate.transitTimeDays * 24 * 60 * 60 * 1000,
      ).toISOString();
      ...
      */
      ...
      alert(
        "Database connection failed. Please ensure Firebase Data Connect Emulator is running or cloud is accessible.",
      );
    } finally {
      setIsBooking(null);
    }
  ...
  const filteredAndSortedRates = useMemo(() => {
    ...
    result.sort((a, b) => {
      let aVal =
        sortKey === "total" ? convertAmount(a.total, a.currency) : a[sortKey];
      let bVal =
        sortKey === "total" ? convertAmount(b.total, b.currency) : b[sortKey];
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-100/50 backdrop-blur-md border-b border-white/50 text-[10px] uppercase tracking-widest text-slate-500 font-bold">
              ...
              <th className="py-4 px-6 whitespace-nowrap text-center">
                Action
              </th>
            </tr>
          </thead>
            ...
            {filteredAndSortedRates.map((rate) => {
              ...
              return (
                <React.Fragment key={rate.id}>
                  <tr
                    onClick={() =>
                      setExpandedRowId(isExpanded ? null : rate.id)
                    }
                        ...
                        <div className="flex gap-12">
                            ...
                            <div className="space-y-3">
                              {rate.surcharges.map((s, i) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span className="w-20 text-[10px] font-bold text-slate-600 uppercase">
                                      {s.name}
                                    </span>
                          ...
                          <div className="w-[300px] border-l border-white/60 pl-8 flex flex-col justify-center">
                            ...
                            <div className="mt-4 p-3 bg-white rounded-lg border border-slate-100 text-xs text-slate-500 italic">
                              "This rate is subject to space and equipment
                              availability. Peak Season Surcharge (PSS) may vary
                              at time of sailing."
                            </div>
                          </div>
                        </div>
