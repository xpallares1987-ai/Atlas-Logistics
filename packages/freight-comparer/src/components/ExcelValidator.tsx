...
export default function ExcelValidator({ columns, sheetName }: ExcelValidatorProps) {
  const missingRequired = columns.filter(c => c.required && !columns.find(col => col.name === c.name)?.found);
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 mt-3 animate-fade-in">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${missingRequired.length === 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
        ...
        {columns.map((col, idx) => (
          <div key={idx} className="flex items-center justify-between bg-slate-800/50 p-2 rounded border border-slate-700/50">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className={`text-[10px] truncate ${col.found ? 'text-slate-200' : 'text-slate-500'}`}>
                {col.name}
              </span>
            </div>
            {col.aliasUsed && (
