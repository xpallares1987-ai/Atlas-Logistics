...
export default function UnitTestRunner() {
  ...
  const totalAssertionsCount = results.reduce(
    (acc, r) => acc + r.assertions.length,
    0,
  );
  const passedAssertionsCount = results.reduce(
  ...
  return (
    <div
      id="unit-test-compliance-runner"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Client-side verification of spreadsheet parsing accuracy and
              multi-route searching sifting rules.
            </p>
          </div>
        </div>
      ...
      <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between text-xs text-slate-600">
        ...
        <div className="flex items-center gap-4 font-mono text-[10px]">
          <span className="flex items-center gap-1">
            ...
            Suite:{" "}
            {results.reduce((sum, r) => sum + r.durationMs, 0).toFixed(2)}ms
          </span>
      ...
      <div className="divide-y divide-slate-100">
        {results.map((suite) => {
          ...
          const scorePercent = Math.round(
            (passedCount / suite.assertions.length) * 100,
          );
            ...
            <div key={suite.id} className="transition hover:bg-slate-50/20">
              ...
              >
                <div className="space-y-1 min-w-0 pr-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      className={`text-[9px] font-mono leading-none px-1.5 py-0.5 rounded-full font-bold ${
                        suite.passed
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <div className="text-right font-mono hidden sm:block">
                    <div className="text-xs font-bold text-slate-800">
                      {passedCount}/{suite.assertions.length} asserts
                    </div>
                    <div className="text-[9px] text-slate-400 font-semibold">
                      {scorePercent}% pass-rate
                    </div>
                  </div>
              ...
              {isExpanded && (
                <div className="px-5 pb-5 pt-0 border-t border-slate-50/50 bg-slate-50/30">
                  <div
                    id={`assertions-detail-box-${suite.id}`}
                      ...
                      {suite.assertions.map((a, aIdx) => (
                        ...
                        >
                          ...
                          <div className="space-y-0.5">
                            ...
                            <span
                              className={`text-[10px] block font-mono ${a.passed ? "text-slate-450" : "text-rose-650"}`}
                            >
                              {a.message}
                            </span>
                          </div>
