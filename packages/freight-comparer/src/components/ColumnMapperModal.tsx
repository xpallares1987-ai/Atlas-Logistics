...
import { CheckCircle, X, Save, AlertTriangle, GripVertical } from "lucide-react";
import { DndContext, useDraggable, useDroppable, DragEndEvent, rectIntersection } from "@dnd-kit/core";
  ...
  return (
    <div
      ...
      className={`flex items-center gap-2 p-2 px-3 text-sm bg-white border rounded-md shadow-sm cursor-grab active:cursor-grabbing hover:border-indigo-400 ${
        isDragging ? 'border-indigo-500 shadow-md ring-2 ring-indigo-200' : 'border-slate-200'
      }`}
    >
    ...
    <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex flex-col gap-3">
      ...
      <div 
        ...
        className={`min-h-[46px] flex items-center rounded-md border-2 border-dashed p-1 transition-colors ${
          ...
          mappedHeader ? 'border-transparent bg-slate-50' :
          field.required ? 'border-amber-300 bg-amber-50/50' : 'border-slate-200 bg-slate-50'
        }`}
      >
        {mappedHeader ? (
...
export default function ColumnMapperModal({ rawHeaders, onSave, onCancel }: ColumnMapperModalProps) {
  ...
  const isReady = REQUIRED_FIELDS.filter(f => f.required).every(f => !!mapping[f.key]);
          ...
          <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-xl">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Drag and drop your spreadsheet columns to match our required fields.
              </p>
            </div>
              ...
              {availableHeaders.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {availableHeaders.map((header) => (
                    <DraggableHeader key={`header-${header}`} id={`drag-${header}`} text={header} />
                  ))}
                </div>
              )}
            </div>
            ...
            <span className="text-xs text-slate-500">
              {isReady ? (
                ...
                <span className="text-amber-600 flex items-center gap-1">
                  <AlertTriangle className="w-4 h-4" /> Please map all required fields (*)
                </span>
              )}
            </span>
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`px-5 py-2 text-sm font-bold text-white rounded flex items-center gap-2 transition-colors ${
                  isReady ? "bg-indigo-600 hover:bg-indigo-700 shadow-md" : "bg-slate-300 cursor-not-allowed"
                }`}
              >
