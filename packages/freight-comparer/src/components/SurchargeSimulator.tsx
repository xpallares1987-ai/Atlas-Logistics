...
const DEFAULT_SURCHARGES: Omit<SurchargeRule, 'id' | 'active'>[] = [
  { name: 'PSS Red Sea', type: 'PSS', amount: 500, currency: 'USD', calcMethod: 'PER_TEU' },
  { name: 'BAF Q3 Adjust', type: 'BAF', amount: 250, currency: 'USD', calcMethod: 'PER_TEU' },
  { name: 'CAF Market Fluctuations', type: 'CAF', amount: 5, currency: 'USD', calcMethod: 'PERCENTAGE' },
];

export default function SurchargeSimulator({ activeSurcharges, onChange, lang }: Props) {
  const [isOpen, setIsOpen] = useState(false);

  const toggleSurcharge = (id: string) => {
    onChange(
      activeSurcharges.map(s => s.id === id ? { ...s, active: !s.active } : s)
    );
  };

  const addDefaultSurcharge = (surchargeInfo: Omit<SurchargeRule, 'id' | 'active'>) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 mb-6 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 text-sm">Pricing Engine: Dynamic Surcharges</h3>
            <p className="text-xs text-slate-500">Simulate BAF, CAF, and PSS impacts on freight costs</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
              ...
              {activeSurcharges.length === 0 ? (
                ...
                <div className="space-y-2">
                  {activeSurcharges.map(s => (
                    <div key={s.id} className={`flex items-center justify-between p-3 rounded-lg border ${s.active ? 'bg-white border-indigo-200 shadow-sm' : 'bg-transparent border-slate-200 opacity-60'} transition-all`}>
                      <div className="flex items-center gap-3">
                        <button onClick={() => toggleSurcharge(s.id)} className="text-indigo-600 hover:text-indigo-700 focus:outline-none">
                          {s.active ? <CheckCircle2 className="w-5 h-5" /> : <Circle className="w-5 h-5 text-slate-300" />}
                        </button>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-sm text-slate-900">{s.name}</span>
                            <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded">{s.type}</span>
                          </div>
                          <div className="text-xs text-slate-500 mt-0.5">
                            {s.calcMethod === 'PERCENTAGE' ? `${s.amount}% of Ocean Freight` : `+ ${s.amount} ${s.currency} per BL`}
                          </div>
                        </div>
                      </div>
                      <button onClick={() => removeSurcharge(s.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors">
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded">{template.type}</span>
                      </div>
                      <div className="text-xs text-slate-500 mt-0.5">
                        {template.calcMethod === 'PERCENTAGE' ? `${template.amount}%` : `+${template.amount} ${template.currency}`}
                      </div>
                    </div>
