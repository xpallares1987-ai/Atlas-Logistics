...
const ENCRYPTED_JS_PATH = path.resolve(__dirname, '..', 'assets', 'datos_encrypted.js');
...
try {
  ...
  if (!match) {
    console.error('❌ Could not parse window.DATA_ENCRYPTED from', ENCRYPTED_JS_PATH);
    process.exit(1);
  }

  const payload = JSON.parse(match[1]);
  if (!payload.salt || !payload.iv || !payload.data) {
    console.error('❌ Encrypted payload is missing required fields (salt, iv, or data)');
    process.exit(1);
  }
  ...
  if (rawData.length < 16) {
    console.error('❌ Encrypted data is too short to contain authentication tag');
    process.exit(1);
  }
