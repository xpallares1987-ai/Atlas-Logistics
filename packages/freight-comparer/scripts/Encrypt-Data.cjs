...
const jsonSource = path.resolve(
  __dirname,
  "..",
  "assets",
  "Tablas_Comparativas_Datos.json",
);
...
if (fs.existsSync(jsonSource)) {
  ...
  console.error(
    "❌ Could not locate unencrypted data source (checked json, ts, fallback)",
  );
  process.exit(1);
}
...
try {
  ...
  const encrypted = Buffer.concat([
    cipher.update(minifiedData, "utf8"),
    cipher.final(),
  ]);
  ...
  console.log(
    `   Final file size: ${(outputScript.length / 1024).toFixed(2)} KB`,
  );
} catch (err) {
