...
export default function ComparisonTable({
  ...
  const formatCur = (v: number | undefined) => {
    ...
    const converted = convertCurrency(v, "USD", currency);
    return formatCurrency(converted, currency);
  };

  // Excel / CSV / PDF Export Handlers
  ...
  const handleExportExcel = async () => {
    ...
    const excelRows = filteredRates.map((rate) => ({
    ...
    const worksheet = workbook.addWorksheet("Carrier Analysis Matrix");
    ...
    saveAs(new Blob([buffer]), "FreightSync_Ocean_Rates_Comparative.xlsx");
  };
  ...
  return (
    <div id="comparison-analysis-panel" className="space-y-6">
      ...
      {/* Winner Highlight Section (Phase 3 - Side by Side Comparison) */}
      {(cheapestRate || fastestRate) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {cheapestRate && (
            <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-5 shadow-md text-white border border-emerald-400 relative overflow-hidden group">
        ...
        <div className="p-5 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/30">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition cursor-pointer"
          ...
          <div className="overflow-x-auto">
            ...
            >
              ...
              <tbody className="divide-y divide-slate-150">
                ...
                {/* 2.5 Simulated Surcharges Row */}
                <tr className="bg-indigo-50/30">
                  <td className="p-3 font-semibold text-indigo-700 bg-indigo-100/40 border-r border-indigo-200 pl-4">
                    Active Simulations
                  </td>
                  {columnsList.map((col, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col items-end justify-center">
                          ...
                          {isFastest && !isBest && (
                            <span className="text-[9px] font-black bg-white text-indigo-800 px-1.5 py-0.5 rounded mt-1 inline-block">
                              ⚡ MÁS RÁPIDA
                            </span>
                          )}
                        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-center">
                        <button
                          onClick={() => generateCustomerQuote(rate)}
                          className="px-2 py-1 text-[10px] bg-slate-800 text-white font-bold uppercase rounded hover:bg-slate-700 transition"
                        >
                          Quote
                        </button>
                      </td>
