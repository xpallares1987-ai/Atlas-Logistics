...
export default function RouteAnalyticsChart({
  ...
  const [data, setData] = useState<{ months: string[]; marketRates: number[]; atlasRates: number[] } | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    setIsLoading(true);
    drizzleRateService.fetchAnalytics(origin, destination)
      .then((res) => {
        setData(res);
      })
      .catch((err) => {
  ...
  const { chartData, chartOptions, trend, savingsPercent } = useMemo(() => {
    if (!data) return { chartData: null, chartOptions: null, trend: "flat", savingsPercent: 0 };
    ...
    const savingsPercent = currentMarketRate > 0 ? Math.round(((currentMarketRate - convertedCurrentLowest) / currentMarketRate) * 100) : 0;

    const trend = savingsPercent > 0 ? "down" : savingsPercent < 0 ? "up" : "flat";
    ...
    <div className="w-full bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl relative overflow-hidden mb-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-black text-white tracking-tight">Market Price Trends</h3>
            <p className="text-xs font-medium text-slate-400">
              6-Month historical analysis for <span className="text-slate-300 font-bold">{origin.split(",")[0]}</span> to <span className="text-slate-300 font-bold">{destination.split(",")[0]}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 bg-slate-950/50 p-3 rounded-xl border border-slate-800/50">
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Atlas Advantage</p>
            <div className="flex items-center gap-2">
              <span className="text-xl font-black text-white">
                {activeCurrency} {convertAmount(currentLowestRate).toLocaleString()}
              </span>
              <div className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${
                trend === 'down' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 
                trend === 'up' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 
      ...
      <div className="w-full h-[220px] relative z-10 flex items-center justify-center transition-opacity duration-500">
        {isLoading || !chartData ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Loading Analytics...</p>
          </div>
        ) : (
