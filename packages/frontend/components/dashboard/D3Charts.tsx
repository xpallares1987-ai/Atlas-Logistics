...
export default function D3DashboardCharts() {
  ...
  useEffect(() => {
    ...
    svg.append('g')
      ...
      .selectAll('line').attr('stroke', '#2d3748').attr('stroke-dasharray', '3,3');
    ...
    svg.selectAll('.tick text').attr('fill', '#a0aec0').attr('font-size', '12px');
    ...
    gradient.append('stop').attr('offset', '0%').attr('stop-color', '#818cf8').attr('stop-opacity', 0.4);
    gradient.append('stop').attr('offset', '100%').attr('stop-color', '#818cf8').attr('stop-opacity', 0);

    // Area
    const area = d3.area<{month: string, volume: number}>()
      .x(d => x(d.month) || 0)
      .y0(chartHeight)
      .y1(d => y(d.volume))
    ...
    // Line
    const line = d3.line<{month: string, volume: number}>()
      .x(d => x(d.month) || 0)
      .y(d => y(d.volume))
      .curve(d3.curveMonotoneX);
    ...
    // Tooltip interaction
    const tooltip = d3.select(volumeRef.current).append('div')
      .attr('class', 'absolute invisible bg-[#111114] border border-gray-800 rounded px-3 py-2 text-white text-xs shadow-lg pointer-events-none transition-opacity duration-200 z-50');
    ...
    svg.selectAll('.dot')
      ...
      .on('mouseover', (event, d) => {
        ...
        tooltip.html(`<strong>${d.month}</strong><br/>Volumen: ${d.volume} TEUs`)
  ...
  useEffect(() => {
    ...
    const svg = d3.select(profitRef.current)
      .append('svg')
      .attr('width', width)
      .attr('height', height)
      .append('g')
    ...
    svg.append('g')
      .call(d3.axisLeft(y).tickSize(0).tickPadding(10).ticks(5).tickFormat(d => `$${(Number(d)/1000)}k`))
      .select('.domain').remove();
    svg.selectAll('.tick text').attr('fill', '#a0aec0').attr('font-size', '12px');

    const tooltip = d3.select(profitRef.current).append('div')
      .attr('class', 'absolute invisible bg-[#111114] border border-gray-800 rounded px-3 py-2 text-white text-xs shadow-lg pointer-events-none transition-opacity duration-200 z-50');
    ...
    svg.selectAll('.bar')
      ...
      .on('mouseover', (event, d) => {
        ...
        tooltip.html(`<strong>${d.carrier}</strong><br/>Beneficio: $${d.profit.toLocaleString()}`)
          .style('visibility', 'visible');
      })
      .on('mousemove', (event) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-[#16161A] p-6 rounded-xl border border-gray-800/40 w-full flex flex-col h-[350px]">
        <div className="mb-2">
          <h3 className="text-sm font-medium text-white">Volumen de Embarques (TEUs)</h3>
          <p className="text-xs text-gray-500 mt-1">Tendencia mensual de volumen</p>
        </div>
      ...
      <div className="bg-[#16161A] p-6 rounded-xl border border-gray-800/40 w-full flex flex-col h-[350px]">
        <div className="mb-2">
          <h3 className="text-sm font-medium text-white">Rentabilidad por Naviera</h3>
          <p className="text-xs text-gray-500 mt-1">Margen consolidado por transportista principal</p>
        </div>
