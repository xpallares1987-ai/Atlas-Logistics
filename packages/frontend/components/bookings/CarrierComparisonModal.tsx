...
export default function CarrierComparisonModal({ isOpen, onClose, pol, pod, onSelect }: CarrierComparisonModalProps) {
  if (!isOpen) return null;

  const carriers = [
    { name: 'MSC', transitTime: '21 días', price: 1850, validUntil: '2023-11-30', direct: true },
    { name: 'Maersk', transitTime: '24 días', price: 1750, validUntil: '2023-11-15', direct: false },
    { name: 'CMA CGM', transitTime: '20 días', price: 1900, validUntil: '2023-12-05', direct: true },
    { name: 'Hapag-Lloyd', transitTime: '28 días', price: 1600, validUntil: '2023-10-31', direct: false },
  ];
      ...
      <div className="bg-[#111114] border border-gray-800 rounded-xl w-full max-w-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-5 border-b border-gray-800 bg-[#16161A]">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Mostrando opciones disponibles para la ruta <span className="font-mono text-gray-200">{pol || 'POL'}</span> → <span className="font-mono text-gray-200">{pod || 'POD'}</span>
            </p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-white transition bg-gray-800/50 hover:bg-gray-700 p-2 rounded-lg">
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              <tbody className="divide-y divide-gray-800/50 bg-[#0A0A0B]">
                {carriers.map((carrier, idx) => (
                  <tr key={idx} className="hover:bg-gray-800/20 transition-colors">
                    <td className="p-3 font-medium text-gray-200">{carrier.name}</td>
                    <td className="p-3 text-sm text-gray-400 flex items-center">
                      <Clock className="w-3.5 h-3.5 mr-1" /> {carrier.transitTime}
                    </td>
                    <td className="p-3 text-sm text-gray-400">
                      {carrier.direct ? (
                         <span className="text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded text-[10px] font-bold">DIRECTO</span>
                      ) : (
                         <span className="text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded text-[10px] font-bold">TRANSBORDO</span>
                      )}
                    </td>
                    <td className="p-3 text-sm text-gray-400">{carrier.validUntil}</td>
        ...
        <div className="p-4 border-t border-gray-800 bg-[#16161A] text-xs text-gray-500 flex justify-between items-center">
          <p className="flex items-center">
             ...
             Tarifas referenciales sujetas a disponibilidad de equipo (GRI aplicable).
          </p>
          <button onClick={onClose} className="px-4 py-2 hover:text-white transition">Cerrar</button>
        </div>
      </div>
    </div>
  );
