...
export default function AddressSelector({
  ...
  const selectedAddress = addresses.find(
    (addr) =>
      addr.name.toLowerCase() === value.toLowerCase() || addr.id === value,
  );
  ...
  const handleCreateOnTheFly = () => {
    ...
    const newType =
      allowedTypes && allowedTypes.length > 0 ? allowedTypes[0] : "Cliente";
    const newAddress: CompanyAddress = {
      ...
      phone: "N/A",
      email:
        "contacto@" + searchTerm.toLowerCase().replace(/\s+/g, "") + ".com",
  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${className}`}>
      ...
      >
        <span
          className={`truncate ${!selectedAddress && !value ? "text-gray-500" : "text-white font-medium"}`}
        >
          {displayLabel}
        </span>
        <ChevronDown
          className={`w-4 h-4 ml-2 transition-transform duration-200 text-gray-500 shrink-0 ${isOpen ? "rotate-180 text-blue-500" : ""}`}
        />
      </button>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span
                        className={`text-[9px] font-bold px-1.5 py-0.2 rounded border uppercase shrink-0 ${getTypeStyle(addr.type)}`}
                      >
                        {addr.type}
                      </span>
                    </div>
              ...
              <div className="p-4 text-center">
                ...
                {searchTerm.trim().length > 1 && (
                  ...
                  >
                    ...
                    Registrar &ldquo;{searchTerm}&rdquo; como{" "}
