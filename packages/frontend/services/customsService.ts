...
export class CustomsService {
  public async getHsCodes(search?: string) {
    const url = search
      ? `${API_BASE_URL}/customs/hs-codes?search=${encodeURIComponent(search)}`
      : `${API_BASE_URL}/customs/hs-codes`;
  ...
  public async getDeclarations(shipmentId?: number) {
    const url = shipmentId
      ? `${API_BASE_URL}/customs/declarations?shipment_id=${shipmentId}`
      : `${API_BASE_URL}/customs/declarations`;
    const res = await fetch(url);
  ...
  public async updateDeclarationStatus(
    ...
    const res = await fetch(
      `${API_BASE_URL}/customs/declarations/${id}/status`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
