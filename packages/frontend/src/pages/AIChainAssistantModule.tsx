  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: "assistant",
      content:
        "Hello! I am your AI Supply Chain Assistant. I can help you query logistics data, optimize warehouse routes, predict demurrage risks, and analyze profitability. How can I help you today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiResponse: Message = {
        ...
        type: "assistant",
        content:
          "Lo siento, el backend de IA no está disponible en este momento.",
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, aiResponse]);
    } finally {
          ...
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
            ...
            <ul className="text-xs text-slate-400 space-y-2">
              <li className="flex justify-between">
                <span>Firebase Data Connect</span>{" "}
                <span className="text-emerald-400">Live</span>
              </li>
              <li className="flex justify-between">
                <span>AlloyDB Omni</span>{" "}
                <span className="text-emerald-400">Synced</span>
              </li>
              <li className="flex justify-between">
                <span>Camunda BPM</span>{" "}
      ...
      <div className="flex-1 flex flex-col relative bg-slate-950">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 z-10 scrollbar-hide">
          {messages.map((msg) => (
            ...
            >
              <div
                className={`flex gap-4 max-w-[80%] ${msg.type === "user" ? "flex-row-reverse" : "flex-row"}`}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border shadow-lg ${msg.type === "user" ? "bg-indigo-600 border-indigo-500" : "bg-slate-800 border-slate-700"}`}
                >
                  {msg.type === "user" ? (
                ...
                <div
                  className={`p-4 rounded-2xl shadow-xl backdrop-blur-md text-sm leading-relaxed ${
                    msg.type === "user"
        ...
        <div className="p-6 bg-slate-900/50 border-t border-slate-800/80 backdrop-blur-xl z-10">
          ...
          <div className="text-center mt-3 flex items-center justify-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">
              Enterprise Grade AI • Secure Data Enclave
            </span>
          </div>
        </div>
      </div>
