...
const AVAILABLE_ROLES = ['ADMIN', 'EXECUTIVE', 'MANAGER', 'TEAM LEADER', 'SALES', 'OPERATOR', 'GUEST'];
...
export default function UserManagement() {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'OPERATOR': return <ShieldCheck className="w-4 h-4 text-blue-500" />;
      case 'CLIENT': return <Shield className="w-4 h-4 text-emerald-500" />;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Team & Permissions</h2>
          <p className="text-slate-500 text-sm mt-1">Manage platform access and role-based privileges.</p>
        </div>
        <button onClick={fetchUsers} className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
          Refresh List
        </button>
      </div>
