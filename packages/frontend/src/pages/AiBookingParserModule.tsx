...
export default function AiBookingParserModule() {
  ...
  const handleFile = (selectedFile: File) => {
    ...
    if (!validTypes.includes(selectedFile.type)) {
      setError(
        "Formato no soportado. Por favor sube un PDF, Word o Imagen (JPG/PNG).",
      );
      return;
    }
      ...
      reader.onload = async (e) => {
        ...
        eventSource.onmessage = (event) => {
          ...
          if (data.shipmentId === shipment.id && data.extractedData) {
            ...
            setSuggestions({
              shipper: data.extractedData.shipperText
                ? {
                    exactMatch: true,
                    company: { id: "temp", name: "Identificado por IA" },
                  }
                : null,
              consignee: data.extractedData.consigneeText
                ? {
                    exactMatch: true,
                    company: { id: "temp", name: "Identificado por IA" },
                  }
      ...
      <header className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Extrae automáticamente instrucciones de embarque usando Gemini 1.5
            Pro
          </p>
        </div>
      </header>
          ...
          {!file ? (
            ...
            >
              ...
              <p className="text-slate-500 text-center max-w-sm mb-8">
                Soporta instrucciones de embarque en formato PDF, Word o
                Imágenes escaneadas.
              </p>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {!parsedData && !isParsing && !error && (
              <div className="h-full flex flex-col items-center justify-center text-slate-400">
                ...
                <p>
                  Sube un documento y ejecuta el parser para ver los resultados
                  aquí.
                </p>
              </div>
            )}
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      ...
                      {suggestions?.shipper?.exactMatch ? (
                        <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400">
                          ...
                          <span>
                            Match en BD:{" "}
                            <b>{suggestions.shipper.company.name}</b>
                          </span>
                        </div>
                      ) : (
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      ...
                      {suggestions?.consignee?.exactMatch ? (
                        <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400">
                          ...
                          <span>
                            Match en BD:{" "}
                            <b>{suggestions.consignee.company.name}</b>
                          </span>
                        </div>
                      ) : (
                        <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      ...
                      <input
                        ...
                        defaultValue={parsedData.portOfLoading}
                      />
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      <label className="text-xs font-medium text-slate-500 mb-1 block">
                ...
                <section>
                  ...
                  <div className="space-y-4">
                      ...
                      parsedData.containers.length > 0 ? (
                        <ul className="space-y-2">
                          {parsedData.containers.map((c, i) => (
                            <li
                              key={i}
                              className="flex justify-between text-sm bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-700"
                            >
                        ...
                        <p className="text-sm text-slate-500">
                          No se encontraron contenedores específicos (¿Es LCL?)
                        </p>
                      )}
                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      <label className="text-xs font-medium text-slate-500 mb-2 block">
                        Mercancía (Commodities)
                      </label>
                      {parsedData.commodities &&
                      parsedData.commodities.length > 0 ? (
                        <ul className="space-y-2">
                          {parsedData.commodities.map((c, i) => (
                            <li
                              key={i}
                              className="text-sm bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700"
                            >
                        ...
                        <p className="text-sm text-slate-500">
                          No se extrajo detalle de mercancía.
                        </p>
                      )}
                    </div>
                  </div>
