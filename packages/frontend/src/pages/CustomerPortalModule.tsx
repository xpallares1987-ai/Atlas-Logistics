  ...
  const downloadHBL = async (shipment: any) => {
    try {
      const hblData = {
        ...
        commodities: [{ description: "General Cargo", pieces: 100, grossWeightKg: 5000, volumeCbm: 15 }]
      };
      
      const res = await fetch(`${API_URL.replace('/api', '')}/api/documents/hbl`, {
        method: 'POST',
            ...
            <div key={shipment.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
              <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between gap-6 items-start md:items-center">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                  <button onClick={() => downloadHBL(shipment)} className="flex-1 md:flex-none px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-sm rounded-lg transition-colors flex items-center justify-center gap-2 border border-indigo-100">
