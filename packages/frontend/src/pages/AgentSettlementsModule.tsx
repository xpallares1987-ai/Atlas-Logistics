...
export default function AgentSettlementsModule() {
  ...
  const { data, isLoading } = useApiQuery<AgentSettlement[]>(["agent-settlements"], "/agent-settlements");
  ...
  const totalPending = settlements.filter(s => s.status === "Pending").reduce((acc, curr) => acc + curr.netBalance, 0);
  const totalPaid = settlements.filter(s => s.status === "Paid").reduce((acc, curr) => acc + curr.netBalance, 0);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-slate-200 relative overflow-hidden rounded-3xl border border-white/10 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-400 mt-2 text-sm md:text-base font-medium max-w-xl">
              Track global partner profit shares, automate periodic statement reconciliation, and monitor inter-company financial flows.
            </p>
          </div>
          <button 
            onClick={generateSettlement}
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl shadow-lg shadow-indigo-500/20 transition-all active:scale-95 flex items-center gap-2 border border-indigo-500/50"
          >
            <RefreshCw className="w-5 h-5" /> Generate Statement
          </button>
        </div>
      </div>
      ...
      <div className="px-4 md:px-8 py-6 z-10 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 shrink-0">
        <motion.div whileHover={{ y: -4 }} className="bg-white/5 border border-white/10 p-5 rounded-2xl relative overflow-hidden backdrop-blur-xl">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center">
              <Clock className="w-5 h-5 text-amber-400" />
            </div>
            <span className="text-xs font-bold px-2 py-1 bg-amber-500/10 text-amber-300 rounded-lg uppercase tracking-wider">Unsettled</span>
          </div>
        ...
        <motion.div whileHover={{ y: -4 }} className="bg-white/5 border border-white/10 p-5 rounded-2xl relative overflow-hidden backdrop-blur-xl">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold px-2 py-1 bg-emerald-500/10 text-emerald-300 rounded-lg uppercase tracking-wider">Cleared</span>
          </div>
          <p className="text-sm font-medium text-slate-400 mb-1">Total Paid (YTD)</p>
          <h3 className="text-3xl font-black text-white">${totalPaid.toLocaleString()}</h3>
        </motion.div>

        <motion.div whileHover={{ y: -4 }} className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 p-5 rounded-2xl relative overflow-hidden backdrop-blur-xl">
          <div className="flex items-center justify-between mb-4">
            <div className="w-10 h-10 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center">
              <TrendingUp className="w-5 h-5 text-indigo-400" />
            </div>
            <span className="text-xs font-bold px-2 py-1 bg-indigo-500/20 text-indigo-300 rounded-lg uppercase tracking-wider">Volume</span>
          </div>
          <p className="text-sm font-medium text-indigo-200/70 mb-1">Active Agents</p>
          <h3 className="text-3xl font-black text-white">{new Set(settlements.map(s => s.agentName)).size}</h3>
        </motion.div>
      </div>
          ...
          <div className="p-4 border-b border-white/10 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white/5">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-white/10 rounded-xl py-2 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
              />
            </div>
          </div>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-2">
                          <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1.5 ${getStatusColor(settlement.status)}`}>
                            {getStatusIcon(settlement.status)} {settlement.status}
                          </span>
                        </div>
                        <span className="text-xs font-mono text-slate-400">{settlement.statementNumber}</span>
                      </div>
                      <h4 className="text-lg font-bold text-white mb-1 truncate">{settlement.agentName || "Unknown Agent"}</h4>
                      <p className="text-xs text-slate-500 flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5" />
                        {new Date(settlement.periodStart).toLocaleDateString(undefined, { month: 'short', day: 'numeric'})} - {new Date(settlement.periodEnd).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric'})}
                      </p>
                    </div>
                    
                    <div className="mt-4 pt-4 border-t border-white/10 flex items-end justify-between">
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Net Balance</p>
                        <p className="text-xl font-black text-white">
                          {new Intl.NumberFormat("en-US", { style: "currency", currency: settlement.currency, maximumFractionDigits: 0 }).format(settlement.netBalance)}
                        </p>
                      </div>
                      <button className="w-8 h-8 rounded-full bg-white/5 hover:bg-indigo-500/20 text-slate-400 hover:text-indigo-400 flex items-center justify-center transition-colors border border-transparent hover:border-indigo-500/30">
