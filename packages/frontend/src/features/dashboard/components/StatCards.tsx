...
export function StatCards() {
  ...
  const stats = [
    ...
    },
    {
      id: 3,
      name: 'Total Revenue',
      value: statsData ? `$${(statsData.totalRevenue / 1000000).toFixed(1)}M` : '...',
      change: '+8.2%',
      trend: 'up',
      icon: TrendingUp,
      ...
      {stats.map((stat) => {
        ...
        return (
          ...
          >
            ...
            <div className={`absolute -inset-4 bg-gradient-to-r ${stat.color} opacity-0 group-hover:opacity-10 blur-2xl transition-opacity duration-500`} />
            
            <div className={`p-4 rounded-xl ${stat.bgColor} border border-white/5 shadow-inner`}>
            ...
            <div className="z-10">
              ...
              <div className="flex items-center mt-1">
                <span className={`text-xs font-semibold ${stat.trend === 'up' ? 'text-emerald-400' : stat.trend === 'down' ? 'text-rose-400' : 'text-amber-400'}`}>
                  {stat.change}
                </span>
              </div>
            </div>
