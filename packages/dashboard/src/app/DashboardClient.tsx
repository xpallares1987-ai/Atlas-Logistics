...
import type { MappingState, ColumnMapping, UploadedFile } from '../types/dashboard';
...
export default function DashboardClient() {
        ...
        if (res.ok) {
          ...
          if (data.files && data.files.length > 0) {
            ...
            for (const fileUrl of data.files) {
              ...
              if (store.uploadedFiles.some(f => f.name === fileName)) continue;
              ...
              try {
                ...
                const file = new File([blob], fileName, { type: fileRes.headers.get('content-type') || 'text/csv' });
                ...
                if (detectedType === 'operational') store.addOperational(parsed.rows as never, uploadedFile);
                else if (detectedType === 'financial') store.addFinancial(parsed.rows as never, uploadedFile);
  ...
  const handleFilesReady = async (files: File[]) => {
    ...
    try {
      ...
      setMappingState(buildMappingState(file.name, parsed.columns, parsed.rows, detectedType));
    } catch (err) {
  ...
  const handleMappingConfirm = () => {
    ...
    if (reportType === 'operational') store.addOperational(typedRows as never, uploadedFile);
    else if (reportType === 'financial') store.addFinancial(typedRows as never, uploadedFile);
  ...
  return (
    <LogisticsDashboardLayout
      ...
      isSyncing={false}
    >
      <div style={{ position: 'absolute', top: '1rem', right: '1rem', zIndex: 50 }} className="flex items-center gap-4">
        {store.lastImported && (
          ...
          {mappingState ? (
            ...
            <>
              <div className="mb-8 text-center">
                <h2 className="text-2xl font-bold text-white mb-2">Import Report Data</h2>
                <p className="text-slate-400">Upload Operational, Financial, or Exception reports to update the dashboard.</p>
              </div>
              ...
              {store.uploadedFiles.length > 0 && (
                <div className="mt-8 border border-slate-800 rounded-xl bg-slate-900/50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-800 flex justify-between items-center bg-slate-800/20">
                    <h3 className="font-semibold text-sm text-slate-200">Loaded Files</h3>
                    <button onClick={store.clearAll} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1">
                      <Trash2 size={12} /> Clear all data
                    </button>
                  </div>
                  <ul className="divide-y divide-slate-800">
                    {store.uploadedFiles.map(f => (
                      <li key={f.id} className="px-4 py-3 flex justify-between items-center hover:bg-slate-800/30">
                        <div>
                          <div className="font-medium text-sm text-slate-300">{f.name}</div>
                          <div className="text-xs text-slate-500 flex gap-3 mt-1">
                            <span className={`capitalize badge badge--${f.type} px-1.5 py-0.5 text-[10px]`}>{f.type}</span>
      ...
      {activeTab === 'Financial' && (
        <FinancialPanel data={store.financial} />
      )}

      {activeTab === 'Exceptions' && (
        <ExceptionPanel data={store.exceptions} />
      )}
