...
const MOCK_DND_DATA: DndRecord[] = [
  ...
  {
    id: "3",
    containerId: "CMAU9821734",
    type: "Detention",
    status: "Critical",
    location: "Customer Warehouse (Madrid)",
    freeDaysTotal: 5,
    daysConsumed: 8,
    dailyRate: 180,
...
export const DndTracker = () => {
  const [filter, setFilter] = useState<"All" | "Demurrage" | "Detention">(
    "All",
  );
  ...
  return (
    <div className="bg-[#16161A] border border-gray-800 rounded-2xl overflow-hidden flex flex-col shadow-xl">
      {/* Header */}
      ...
      <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
        {filteredData.map((record) => {
          ...
          const daysOverdue = isOverdue
            ? record.daysConsumed - record.freeDaysTotal
            : 0;
          ...
          return (
            <div
              key={record.id}
              className="bg-[#1C1C21] border border-gray-800 rounded-xl p-4 hover:border-gray-700 transition-colors group"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg ${record.type === "Demurrage" ? "bg-indigo-500/10 text-indigo-400" : "bg-purple-500/10 text-purple-400"}`}
                  >
                    {record.type === "Demurrage" ? (
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-gray-200 font-mono tracking-wider">
                      {record.containerId}
                    </h3>
                ...
                <div
                  className={`px-2.5 py-1 rounded border text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 ${getStatusColor(record.status)}`}
                >
                  {record.status === "Safe" && (
              ...
              <div className="space-y-2 bg-[#111114] rounded-lg p-3 border border-gray-800/50">
                <div className="flex justify-between text-xs">
                  ...
                  <span
                    className={`font-mono font-bold ${isOverdue ? "text-red-400" : "text-gray-300"}`}
                  >
