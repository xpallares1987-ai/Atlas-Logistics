  ...
  if (data.length === 0) {
    return (
      <div className="panel-empty">
        ...
        <p>Upload a <strong>Financial</strong> report to see revenue, cost, and profit analytics.</p>
      </div>
    );
  }
    ...
    <div className="financial-panel">
      ...
      <div className="financial-charts">
            ...
            <div className="card-body" style={{ height: 260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyData} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
                  ...
                  <Tooltip {...CHART_STYLE} formatter={(v: any, name: any) => [fmtCurrency(v as number), name as string]} />
                  <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#3b82f6" fill="url(#gradRevenue)" strokeWidth={2} dot={false} />
                  <Area type="monotone" dataKey="cost"    name="Cost"    stroke="#f59e0b" fill="none"              strokeWidth={2} strokeDasharray="4 2" dot={false} />
        ...
        <div className="fin-chart-card">
          ...
          <div className="card-body" style={{ height: 260 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={invoiceStatus} layout="vertical" margin={{ left: 16, right: 16, top: 8, bottom: 8 }}>
                <XAxis type="number" tick={{ fill: '#94a3b8', fontSize: 11 }} tickLine={false} axisLine={false} />
                <YAxis dataKey="label" type="category" tick={{ fill: '#94a3b8', fontSize: 12 }} tickLine={false} axisLine={false} width={60} />
                <Tooltip {...CHART_STYLE} formatter={(v: any) => [v, 'Invoices']} />
      ...
      <div className="fin-table-card">
        <div className="card-header">
          ...
          <p>Showing {Math.min(data.length, 50)} of {data.length}</p>
        </div>
              ...
              {data.slice(0, 50).map((row, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td className="text-slate-400 text-xs">{row.invoice_number ?? '—'}</td>
                    <td className="text-xs">{row.invoice_date ?? '—'}</td>
                    <td className="text-xs">{row.due_date ?? '—'}</td>
                    <td className="text-right text-xs text-blue-400">{row.revenue != null ? fmtCurrency(row.revenue) : '—'}</td>
                    <td className="text-right text-xs text-amber-400">{row.cost != null ? fmtCurrency(row.cost) : '—'}</td>
                    <td className={`text-right text-xs font-semibold ${profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {fmtCurrency(profit)}
                    </td>
                    <td>
                      <span className={`status-badge ${row.paid === true ? 'status-badge--success' : row.paid === false ? 'status-badge--warning' : 'status-badge--muted'}`}>
                        {row.paid === true ? 'Paid' : row.paid === false ? 'Pending' : '—'}
                      </span>
                    </td>
                  </tr>
                );
