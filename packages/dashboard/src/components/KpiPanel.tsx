...
import { TrendingUp, TrendingDown, Minus, Package, Clock, AlertTriangle, DollarSign, BarChart2, Zap, DatabaseZap, UploadCloud } from 'lucide-react';
...
function KpiCard({ label, value, sub, trend, trendLabel, icon, variant = 'default' }: KpiCardProps) {
  const TrendIcon = trend === 'up' ? TrendingUp : trend === 'down' ? TrendingDown : Minus;
  const trendClass = trend === 'up' ? 'kpi-trend--up' : trend === 'down' ? 'kpi-trend--down' : 'kpi-trend--neutral';
...
const LANE_COLORS = ['#3b82f6', '#8b5cf6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#84cc16'];
  ...
  if (isEmpty && !liveKpis) {
    return (
      <div className="panel-empty">
        ...
        <p>Upload an <strong>Operational</strong> or <strong>Financial</strong> report to see KPIs.</p>
      </div>
    );
  }
        ...
        <div className="mb-8 bg-slate-900 border border-slate-700 rounded-xl p-6 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 flex flex-col justify-between">
              ...
              <p className="text-3xl font-black text-white">{liveKpis.activeMetrics?.totalActiveShipments || 0}</p>
            </div>
            <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 flex flex-col justify-between">
              <p className="text-slate-400 text-xs font-bold uppercase mb-2 flex items-center gap-2"><DollarSign size={14}/> Total Spend</p>
              <p className="text-3xl font-black text-emerald-400">${(liveKpis.activeMetrics?.totalSpend || 0).toLocaleString()}</p>
            </div>
            <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 flex flex-col justify-between">
              <p className="text-slate-400 text-xs font-bold uppercase mb-2 flex items-center gap-2"><BarChart2 size={14}/> Total Volume (TEU)</p>
              <p className="text-3xl font-black text-indigo-400">{liveKpis.activeMetrics?.totalVolume || 0}</p>
            </div>
            <div className="bg-slate-800 p-5 rounded-xl border border-red-900/50 flex flex-col justify-between">
              <p className="text-red-400 text-xs font-bold uppercase mb-2 flex items-center gap-2"><AlertTriangle size={14}/> Demurrage Risk</p>
              <p className="text-3xl font-black text-red-500">{liveKpis.demurrageRisks?.riskCount || 0} Shipments</p>
            </div>
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div className="kpi-grid">
            <KpiCard
              ...
              trendLabel={metrics.onTimePercent >= 90 ? 'On target' : 'Below target'}
            />
            <KpiCard
            ...
            <KpiCard
              ...
              trendLabel={metrics.outstandingInvoices === 0 ? 'All paid' : 'Awaiting payment'}
            />
            <KpiCard
              ...
              trendLabel={metrics.activeExceptions === 0 ? 'All clear' : 'Needs attention'}
            />
          </div>
            ...
            <div className="kpi-chart-card">
              ...
              <div className="card-body" style={{ height: 260 }}>
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <BarChart data={laneData} margin={{ left: 0, right: 8, top: 8, bottom: 40 }}>
                    <XAxis
                      dataKey="lane"
                    ...
                    <Tooltip
                      ...
                      formatter={(v: any) => [`${Number(v).toLocaleString()} kg`, 'Weight']}
                    />
          ...
          <div className="kpi-chart-card" style={{ marginTop: '1.5rem' }}>
            ...
            <div className="card-body" style={{ padding: '1.5rem', overflowX: 'auto' }}>
