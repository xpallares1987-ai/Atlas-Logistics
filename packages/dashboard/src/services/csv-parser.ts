...
const TYPE_SIGNALS: Record<ReportType, string[]> = {
  operational: ['etd', 'eta', 'ata', 'carrier', 'trade_lane', 'mode', 'weight', 'volume'],
  financial:   ['invoice', 'revenue', 'cost', 'profit', 'paid', 'due_date', 'currency'],
  exception:   ['exception', 'severity', 'resolved', 'exception_type'],
};

export function autoDetectReportType(columns: string[]): ReportType | null {
  const normalized = columns.map((c) => c.toLowerCase().replace(/[\s\-]/g, '_'));
  const scores: Record<ReportType, number> = { operational: 0, financial: 0, exception: 0 };

  for (const [type, signals] of Object.entries(TYPE_SIGNALS) as [ReportType, string[]][]) {
    for (const signal of signals) {
      if (normalized.some((c) => c.includes(signal))) scores[type]++;
    }
  }

  const best = (Object.entries(scores) as [ReportType, number][]).sort((a, b) => b[1] - a[1])[0];
