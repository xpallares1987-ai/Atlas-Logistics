...
- **REST Endpoints**:
  ...
  - `/api/bulk-operations`: Hydrostatic draft surveys (6-point drafts, trim & density corrections), IMSBC TML/FMP liquefaction evaluator, IMO Grain Code stability, ASTM Table 54 tanker ullage surveys, 4 bulk PDFs.
...
22. **Hydrostatic Draft Survey & IMSBC TML Formulas**:
    $$DQM = \frac{F_m + 6 M_m + A_m}{8}, \quad \text{Trim} = A_m - F_m, \quad C_1 = \frac{\text{Trim} \times \text{LCF} \times \text{TPC} \times 100}{\text{LBP}}$$
    $$\Delta_{\text{corr}} = (\Delta_{\text{hidro}} + C_1 + C_2) \times \left(\frac{\rho_{\text{agua}}}{1.025}\right), \quad \text{TML} = \text{FMP} \times 0.90$$
...
- **121 Relational Tables** mapped with strict TypeScript schemas in `src/db/schema/*.ts`.
...
- **Custom SQL Triggers**: Automatic sequential code generators (`CLM-YYYY-XXXX`, `CMR-YYYY-XXXXX`, `DUA-YYYY-XXXX`, `OEA-YYYY-XXXX`, `CP-YYYY-XXXX`, `GA-YYYY-XXXX`, `DGD-YYYY-XXXX`, `INS-CERT-YYYY-XXXX`, `BULK-YYYY-XXXX`) and immutable audit logs.
...
- **Vitest Suite**: 82 test suites with 384 unit & integration tests covering all deterministic services and Fastify routes with 100% pass rate.
