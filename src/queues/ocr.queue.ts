...
export async function addOcrJob(jobName: string, data: any) {
  if (isRedisAvailable) {
    ...
    logger.warn(
      `Redis is down. Using in-memory synchronous execution for OCR job: ${jobName}`,
    );
    // Simulate async processing
    setTimeout(() => {
      processOcrJob(data).catch((err) =>
        logger.error(err, "In-memory OCR processing failed"),
      );
    }, 100);
...
export function startOcrWorker() {
  if (!isRedisAvailable) {
    logger.warn(
      "Skipping BullMQ OCR Worker start because Redis is unavailable.",
    );
    return null;
  }
