...
server.register(fastifySwagger, {
  openapi: {
    info: {
      ...
      description:
        "API de grado industrial para la automatización de operativas de Freight Forwarding",
      version: "1.0.0",
    },
    components: {
      securitySchemes: {
    ...
    if (idempotencyKey) {
      ...
      if (expiration && expiration > now) {
        return reply
          ...
          .send({
            error: "Conflicto",
            message: "La petición ya ha sido procesada",
          });
      }
...
export const startServer = async () => {
  try {
    ...
    console.log(
      `🚀 API Atlas Logistics SCM en ejecución: http://0.0.0.0:${port}`,
    );
    console.log(
      `📚 Documentación Swagger disponible en: http://0.0.0.0:${port}/documentation`,
    );
  } catch (err) {
