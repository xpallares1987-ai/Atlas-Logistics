...
const quoteRoutes: FastifyPluginAsyncZod = async (server) => {
    ...
    const newQuote = await db.transaction(async (tx) => {
      ...
      if (options && options.length > 0) {
        const optionsToInsert = options.map(opt => ({
          ...
          margin_percentage: opt.margin_percentage ? String(opt.margin_percentage) : null
        }));
        createdOptions = await tx.insert(quote_options).values(optionsToInsert).returning();
      }
      ...
      return {
        ...
        options: createdOptions.map(o => ({
          ...o,
          margin_percentage: o.margin_percentage ? Number(o.margin_percentage) : null,
  ...
  server.patch('/api/quotes/:id', {
    ...
    if (!updatedQuote) return reply.status(404).send({ error: 'Quote not found' });
