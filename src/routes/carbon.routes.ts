  ...
  fastify.get("/calculations", async (req, reply) => {
    try {
      ...
      const where = and(
        ...
        escapedQuery
          ? or(
              sql`${carbonCalculations.referenceCode} LIKE ${`%${escapedQuery}%`} ESCAPE '\\'`,
              sql`${carbonCalculations.originCity} LIKE ${`%${escapedQuery}%`} ESCAPE '\\'`,
              sql`${carbonCalculations.destinationCity} LIKE ${`%${escapedQuery}%`} ESCAPE '\\'`,
            )
          : undefined,
      );
    ...
    try {
      ...
      const [certificates, total] = await Promise.all([
        db
          ...
          .orderBy(desc(carbonCertificates.issuedAt), desc(carbonCertificates.id))
          .limit(pageSize)
          .offset((page - 1) * pageSize),
        db.select({ value: count() }).from(carbonCertificates).get(),
      ]);
