...
const financialRoutes: FastifyPluginAsyncZod = async (server) => {
  server.get(
    ...
    async (request, reply) => {
      const { shipment_id, type } = request.query as {
        shipment_id?: number;
        type?: "AR" | "AP";
      };
      ...
      const queryResult = await db.query.invoices.findMany({
        where: (invoices, { eq, and }) => {
          const conditions = [];
          if (shipment_id)
            conditions.push(eq(invoices.shipment_id, shipment_id));
  ...
  server.patch(
    ...
    async (request, reply) => {
      ...
      const itemsList = await db
        .select()
        .from(invoice_items)
        .where(eq(invoice_items.invoice_id, updatedInvoice.id));
  ...
  server.get(
    "/api/financial/shipments/:shipmentId/pnl",
    {
      schema: {
        tags: ["Financial"],
