...
export default async function aiRoutes(fastify: FastifyInstance) {
  fastify.post("/trigger-task", async (request, reply) => {
    try {
      const { prompt, shipmentId, documentUrl } = taskSchema.parse(
        request.body,
      );
  ...
  fastify.post("/chat", async (request, reply) => {
    try {
      ...
      let replyText =
        "Entendido. Procesando tu solicitud con los datos del ERP logístico...";

      if (lower.includes("demurrage")) {
        replyText =
          "He analizado los contenedores. Tienes 2 contenedores en ESBCN acercándose al límite de días libres. Te sugiero despacharlos antes del viernes para evitar $400 en demurrage.";
      } else if (
        ...
        replyText =
          "La ocupación actual del almacén de tránsito es del 82%. Tenemos capacidad para el próximo envío de 15 pallets procedente de Shanghai.";
      } else if (lower.includes("route") || lower.includes("ruta")) {
        replyText =
          "La ruta CNSHA -> ESBCN actualmente tiene un tránsito promedio de 32 días. La alternativa por aire tomaría 4 días pero incrementaría el costo en un 450%.";
      }
