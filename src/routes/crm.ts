...
import { customers, contacts, rate_agreements, rate_agreement_items } from '../db/schema.js';
...
const crmRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.post('/api/crm/customers', {
    schema: {
      tags: ['CRM'],
  ...
  fastify.get('/api/crm/customers/:id', {
    schema: {
      tags: ['CRM'],
  ...
  fastify.patch('/api/crm/customers/:id', {
    schema: {
      tags: ['CRM'],
    ...
    if (!updatedCustomer) return reply.status(404).send({ error: 'Customer not found' });
    return {
      ...updatedCustomer,
  ...
  fastify.post('/api/crm/customers/:id/contacts', {
    schema: {
      tags: ['CRM'],
  ...
  // Rate Agreements
  fastify.get('/api/crm/customers/:id/agreements', {
    schema: {
      tags: ['CRM'],
  ...
  fastify.post('/api/crm/customers/:id/agreements', {
    schema: {
      tags: ['CRM'],
    ...
    const newAgreement = await db.transaction(async (tx) => {
      const [agreement] = await tx.insert(rate_agreements)
        .values({
          ...agreementData,
          customer_id: id,
