...
const freightRoutes: FastifyPluginAsyncZod = async (server) => {
  server.post(
    "/api/freight-rates/calculate",
    {
      schema: {
        description:
          "Calculate freight rates with dynamic surcharges from Zeebe DMN",
        tags: ["Freight"],
        security: [{ bearerAuth: [] }],
        body: z.object({
    ...
    async (request, reply) => {
      const { origin, destination, carrier } = request.body as {
        origin: string;
        destination: string;
        carrier: string;
      };
      ...
      try {
        ...
        server.log.warn(
          `Redis cache lookup failed: ${(error as Error).message}`,
        );
      }
      ...
      try {
        ...
        server.log.warn(
          `Fallback defaults for ${carrier}. Error: ${(error as Error).message}`,
        );
      }
      ...
      try {
        ...
        server.log.warn(
          `Failed to write to Redis cache: ${(error as Error).message}`,
        );
      }
