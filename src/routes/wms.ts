...
const wmsRoutes: FastifyPluginAsyncZod = async (server) => {
  ...
  server.post('/api/wms/warehouses', {
    schema: {
      tags: ['WMS'],
  ...
  server.get('/api/wms/stock', {
    schema: {
      ...
      response: {
        200: z.array(stockItemSchema.extend({
          movements: z.array(inventoryMovementSchema).optional()
        }))
      }
    }
  }, async (request, reply) => {
    const { shipment_id, warehouse_id } = request.query as { shipment_id?: number, warehouse_id?: number };
    
    const results = await db.query.stock_items.findMany({
      where: (stock_items, { eq, and }) => {
        const conditions = [];
        if (shipment_id) conditions.push(eq(stock_items.shipment_id, shipment_id));
        if (warehouse_id) conditions.push(eq(stock_items.warehouse_id, warehouse_id));
  ...
  server.post('/api/wms/movements/receive', {
    schema: {
      tags: ['WMS'],
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const [existingItem] = await tx.select().from(stock_items).where(and(...conditions)).limit(1);

      if (existingItem) {
        const [updatedItem] = await tx.update(stock_items)
          .set({ 
      ...
      // Record movement
      const [movement] = await tx.insert(inventory_movements)
        .values({
    ...
    return {
      stock_item: {
        ...result.stockItem,
        weight_kg: result.stockItem.weight_kg ? Number(result.stockItem.weight_kg) : null,
  ...
  server.post('/api/wms/movements/dispatch', {
    schema: {
      tags: ['WMS'],
    ...
    try {
      const result = await db.transaction(async (tx) => {
        const [existingItem] = await tx.select().from(stock_items).where(eq(stock_items.id, body.stock_item_id)).limit(1);
        ...
        if (existingItem.quantity_on_hand < body.quantity) {
          throw new Error(`Insufficient quantity. Available: ${existingItem.quantity_on_hand}`);
        }
