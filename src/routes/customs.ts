...
const customsRoutes: FastifyPluginAsyncZod = async (server) => {
  ...
  server.get(
    ...
    async (request, _reply) => {
      ...
      const queryResult = await db.query.customs_declarations.findMany({
        where: shipment_id
          ? eq(customs_declarations.shipment_id, shipment_id)
          : undefined,
        with: {
          items: true,
        },
  ...
  server.post(
    ...
    async (request, reply) => {
      const body = request.body as z.infer<
        typeof createCustomsDeclarationSchema
      >;
      ...
      const newDeclaration = await db.transaction(async (tx) => {
        ...
        if (items && items.length > 0) {
          ...
          createdItems = await tx
            .insert(customs_declaration_items)
  ...
  server.patch(
    "/api/customs/declarations/:id/status",
    {
      schema: {
        tags: ["Customs"],
    ...
    async (request, reply) => {
      ...
      if (body.submission_date)
        updatePayload.submission_date = new Date(body.submission_date);
      if (body.clearance_date)
        updatePayload.clearance_date = new Date(body.clearance_date);
      ...
      const itemsList = await db
        ...
        .where(
          eq(customs_declaration_items.declaration_id, updatedDeclaration.id),
        );
