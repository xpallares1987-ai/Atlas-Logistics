...
async function seed() {
  ...
  const insertedSurchargeTypes = await db
    .insert(surcharge_types)
    .values(surchargeTypeData)
    .returning();
  ...
  for (const carrier of insertedCarriers) {
    for (const route of routes) {
      ...
      const selectedTypes = insertedSurchargeTypes
  ...
  for (let i = 0; i < 15; i++) {
    const randomCarrier =
      insertedCarriers[Math.floor(Math.random() * insertedCarriers.length)];
    const randomStatus =
      shipmentStatuses[Math.floor(Math.random() * shipmentStatuses.length)];
    const randomMode =
      shipmentModes[Math.floor(Math.random() * shipmentModes.length)];
    const randomType =
