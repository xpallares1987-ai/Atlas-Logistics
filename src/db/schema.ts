...
export const shipments = pgTable('shipments', {
  ...
  shipperAliasId: uuid('shipper_alias_id').references(() => companyBlAliases.id),
  consigneeAliasId: uuid('consignee_alias_id').references(() => companyBlAliases.id),
...
export const lettersOfCredit = pgTable('letters_of_credit', {
  ...
  issuingBankId: uuid('issuing_bank_id').references(() => companies.id).notNull(),
...
export const rates = pgTable('rates', {
  ...
  destinationLocationId: uuid('destination_location_id').references(() => locations.id), // Changed from string
...
export const quotes = pgTable('quotes', {
  ...
  customerId: uuid('customer_id').references(() => companies.id).notNull(), // Changed from string
  originLocationId: uuid('origin_location_id').references(() => locations.id), // Changed from string
  destinationLocationId: uuid('destination_location_id').references(() => locations.id), // Changed from string
...
export const invoices = pgTable('invoices', {
  ...
  partyId: uuid('party_id').references(() => companies.id).notNull(),
...
export const agentSettlements = pgTable('agent_settlements', {
  id: uuid('id').defaultRandom().primaryKey(),
  statementNumber: varchar('statement_number', { length: 100 }).notNull().unique(),
  agentId: uuid('agent_id').references(() => companies.id).notNull(),
...
export const settlementInvoices = pgTable('settlement_invoices', {
  id: uuid('id').defaultRandom().primaryKey(),
  settlementId: uuid('settlement_id').references(() => agentSettlements.id).notNull(),
  invoiceId: uuid('invoice_id').references(() => invoices.id).notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
...
export const companyBlAliasesRelations = relations(companyBlAliases, ({ one }) => ({
...
export const agentSettlementsRelations = relations(agentSettlements, ({ one, many }) => ({
  agent: one(companies, {
...
export const settlementInvoicesRelations = relations(settlementInvoices, ({ one }) => ({
