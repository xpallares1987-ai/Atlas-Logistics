...
export const users = pgTable("users", {
  ...
  tenant_id: varchar("tenant_id", { length: 50 }).default("default").notNull(),
...
export const shipments = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    tenant_id: varchar("tenant_id", { length: 50 })
      .default("default")
      .notNull(),
  ...
  (table) => {
    return {
      tenantIdx: index("ship_tenant_idx").on(table.tenant_id),
...
export const customers = pgTable(
  ...
  (table) => {
    return {
      tenantIdx: index("cust_tenant_idx").on(table.tenant_id),
    };
  },
);
...
export const quotes = pgTable(
  ...
  (table) => {
    return {
      tenantIdx: index("quote_tenant_idx").on(table.tenant_id),
    };
  },
);
