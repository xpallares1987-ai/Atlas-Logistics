...
import { shipments, invoices, containers } from "../db/schema.js";
...
export class ForwardingService {
  ...
  async consolidateShipment(masterId: number, houseId: number) {
    ...
    return {
      success: true,
      message: `Shipment ${houseId} consolidated under ${masterId}`,
    };
  }
  ...
  async calculateProfitAndLoss(shipmentId: number) {
    ...
    return {
      ...
      currency:
        shipmentInvoices.length > 0 ? shipmentInvoices[0].currency : "USD",
    };
  }
  ...
  async addContainer(shipmentId: number, containerData: any) {
    ...
    if (shipment.type === "HBL") {
      throw new Error(
        "Containers can only be added directly to MBL or Direct shipments",
      );
    }
