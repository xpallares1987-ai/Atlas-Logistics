...
async function verifyGovernance() {
  ...
  if (masked.email === testContact.email) throw new Error('Masking failed for email');
  ...
  try {
    ...
    console.warn('⚠️ Constraint bypassed (might happen if foreign key failed first or SQLite check ignored)');
  } catch (e: any) {
