...
const run = async () => {
  ...
  if (args.length === 0) {
    console.error('Error: Ruta del archivo JSON temporal no proporcionada por PowerShell.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Encriptando ${rates.length} tarifas para análisis logístico...`);
