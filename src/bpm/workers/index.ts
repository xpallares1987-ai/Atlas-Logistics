...
export function registerAllWorkers(): void {
  console.log(`[WorkerRegistry] Registering ${workers.length} Zeebe workers...`);

  for (const worker of workers) {
    try {
      worker.register();
    } catch (error) {
      console.error(`[WorkerRegistry] Failed to register worker ${worker.taskType}:`, error);
    }
  }

  console.log(`[WorkerRegistry] ✓ All ${workers.length} workers registered successfully`);
}
