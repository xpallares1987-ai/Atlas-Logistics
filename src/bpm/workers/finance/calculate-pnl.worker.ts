 ...
 */
class CalculatePnLWorker extends AtlasWorker<
  CalculatePnLInput,
  CalculatePnLOutput
> {
  ...
  async execute(job: any): Promise<CalculatePnLOutput> {
    ...
    console.log(
      `[CalculatePnLWorker] Calculating P&L for shipment ${shipmentId}...`,
    );

    // Mocking Revenue
    const estimatedRevenue = expectedCost * 1.3; // 30% margin
    const estimatedProfit = estimatedRevenue - expectedCost;
    const profitable = estimatedProfit > 0;

    console.log(
      `[CalculatePnLWorker] P&L result: Revenue=$${estimatedRevenue.toFixed(2)}, Profit=$${estimatedProfit.toFixed(2)}, Profitable=${profitable}`,
    );
