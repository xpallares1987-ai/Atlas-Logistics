 ...
 */
class OptimizeLoadingWorker extends AtlasWorker<
  OptimizeLoadingInput,
  OptimizeLoadingOutput
> {
  ...
  async execute(job: any): Promise<OptimizeLoadingOutput> {
    ...
    console.log(
      `[OptimizeLoading] Consolidating ${shipments.length} shipments into ${containerType}`,
    );
    ...
    const sorted = [...shipments].sort(
      (a: any, b: any) => b.volumeCbm - a.volumeCbm,
    );
    ...
    if (overweight) {
      throw new AtlasBpmnError(
        "OVERWEIGHT",
        `Container ${containerType} overweight: ${totalWeight}kg exceeds max ${maxWeight}kg`,
      );
    }
    ...
    const optimizationScore = Math.round(
      (utilizationWeight + utilizationVolume) / 2,
    );

    console.log(
      `[OptimizeLoading] Weight: ${utilizationWeight}% | Volume: ${utilizationVolume}% | Score: ${optimizationScore}`,
    );
