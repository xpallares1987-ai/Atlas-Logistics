 ...
 */
class ReceiveCargoWorker extends AtlasWorker<
  ReceiveCargoInput,
  ReceiveCargoOutput
> {
  ...
  async execute(job: any): Promise<ReceiveCargoOutput> {
    ...
    const location = `WH-${String.fromCharCode(65 + Math.floor(Math.random() * 6))}${Math.floor(
      1 + Math.random() * 20,
    )
      .toString()
      .padStart(2, "0")}`;
    ...
    console.log(
      `[ReceiveCargo] Gate-in ${gateInId}: ${containerNumber || "loose cargo"} from truck ${truckPlate || "N/A"}`,
    );
