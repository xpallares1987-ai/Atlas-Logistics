 ...
 */
class CheckRestrictionsWorker extends AtlasWorker<
...
> {
  ...
  async execute(job: any): Promise<CheckRestrictionsOutput> {
    const { hsCode, originCountry, destinationCountry, cargoType } =
      job.variables;
    const restrictions: CheckRestrictionsOutput["restrictions"] = [];

    console.log(
      `[CheckRestrictions] Scanning: HS ${hsCode} (${originCountry} → ${destinationCountry})`,
    );
    ...
    if (
      hsCode.startsWith("72") &&
      originCountry === "CN" &&
      ["ES", "DE", "FR", "IT", "NL"].includes(destinationCountry)
    ) {
      restrictions.push({
        type: "ANTI_DUMPING",
        severity: "MEDIUM",
    ...
    console.log(
      `[CheckRestrictions] Found ${restrictions.length} restrictions (severity: ${overallSeverity})`,
    );
