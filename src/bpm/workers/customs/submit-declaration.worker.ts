 ...
 */
class SubmitDeclarationWorker extends AtlasWorker<
  SubmitDeclarationInput,
  SubmitDeclarationOutput
> {
  readonly taskType = "atlas.customs.submit-declaration";

  async execute(job: any): Promise<SubmitDeclarationOutput> {
    const { referenceNumber, hsCode, originCountry, destinationCountry } =
      job.variables;

    console.log(
      `[SubmitDeclaration] Filing declaration for ${referenceNumber}: HS ${hsCode} (${originCountry} → ${destinationCountry})`,
    );
    ...
    console.log(
      `[SubmitDeclaration] ✓ Declaration ${declarationNumber} submitted successfully`,
    );
