...
const DEFAULT_NOTIFICATIONS: SCMNotification[] = [
  {
    ...
    description:
      "El contenedor de importación HLXU1100223 en el flete FWD-2026-004 ha completado el despacho de aduanas en Barcelona (ESBCN) y cuenta con autorización de levante.",
  ...
  },
  {
    ...
    description:
      "El buque CMA CGM asignado al embarque HAZMAT FWD-2026-003 ha sido demorado debido a congestión de descarga en puerto ESBCN.",
  ...
  },
  {
    ...
    description:
      "El espacio aéreo de alta prioridad para portátiles (FWD-2026-005) ha sido reservado y confirmado.",
...
export function savePreferences(prefs: NotificationPreferences) {
  ...
  window.dispatchEvent(
    new CustomEvent("forwarderos_prefs_changed", { detail: prefs }),
  );
}
...
export function triggerScmNotification(
  ...
  window.dispatchEvent(
    new CustomEvent("forwarderos_new_notification", { detail: newNotif }),
  );
  ...
  if (prefs.emailNotifications && prefs.emailAddress) {
    ...
    window.dispatchEvent(
      new CustomEvent("forwarderos_email_sent", {
