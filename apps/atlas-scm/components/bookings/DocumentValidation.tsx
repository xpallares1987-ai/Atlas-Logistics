...
export default function DocumentValidation({
  ...
  return (
    <div
      className={`p-4 rounded-lg border text-sm w-full mb-4 ${isValid ? "bg-emerald-900/10 border-emerald-800/40" : "bg-red-900/10 border-red-800/40"}`}
    >
      <div className="flex items-start">
        ...
        <div>
          <h4
            className={`font-medium mb-1 ${isValid ? "text-emerald-400" : "text-red-400"}`}
          >
          ...
          {!isValid ? (
            <ul className="list-disc list-inside text-xs text-red-300/80 space-y-1">
              {missingFields.map((field, idx) => (
                <li key={idx}>
                  El campo <strong>{field.label}</strong> es obligatorio.
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-xs text-emerald-300/80">
              Todos los datos requeridos para el Draft HBL están completos.
            </p>
          )}
        </div>
      </div>
