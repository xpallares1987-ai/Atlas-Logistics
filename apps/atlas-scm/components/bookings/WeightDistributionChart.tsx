  ...
  const getCapacity = (type: string) => {
    switch (type) {
      ...
      case "40HC":
        return 28500;
      case "LCL":
        return 10000; // arbitrary metric for groupage max check
      case "AIR":
        return 5000;
      default:
        return 28000;
      ...
      <div className="flex flex-col justify-center">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-gray-300">
            Cargado: <span className="font-mono">{safeWeight} KG</span>
          </span>
        </div>
        {overflowWeight > 0 ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-red-400 font-bold">
              Exceso: <span className="font-mono">{overflowWeight} KG</span>
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">
              Disp.: <span className="font-mono">{remaining} KG</span>{" "}
              <span className="text-[10px]">
                ({Math.round((remaining / capacity) * 100)}%)
              </span>
            </span>
          </div>
        )}
