...
function AuthGuard({ children }: { children?: ReactNode }) {
  ...
  const handleResetPassword = async () => {
    if (!email) {
      setAuthError('Por favor, introduce tu correo electrónico primero para restablecer la contraseña.');
      return;
    }
    setAuthError('');
    setIsSubmitting(true);
    try {
      if (resetPassword) {
        ...
        setAuthError('¡Correo de restablecimiento enviado! Por favor revisa tu bandeja de entrada.');
      }
    } catch (err: any) {
      setAuthError(err.message || 'Fallo al enviar el correo de restablecimiento.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[#09090b] p-4">
        <div className="max-w-md w-full bg-gray-900 border border-gray-800 rounded-xl p-8 shadow-2xl flex flex-col items-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Panel de Envíos</h2>
          <p className="text-gray-400 text-center mb-6">
            Inicia sesión para acceder a tu inteligencia de envíos.
          </p>
          ...
          <form onSubmit={handleEmailAuth} className="w-full mb-6">
            ...
            >
              ...
              {isSignUp ? 'Crear Cuenta' : 'Iniciar Sesión con Correo'}
            </button>
            ...
            <div className="mt-4 text-center">
              ...
              >
                {isSignUp ? '¿Ya tienes una cuenta? Iniciar Sesión' : '¿No tienes cuenta? Regístrate'}
              </button>
            </div>
          </form>
